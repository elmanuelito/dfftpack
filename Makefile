...
# FC=g77
FC=gfortran
FFLAGS=-O5 -fPIC -funroll-loops -fexpensive-optimizations

# Use these lines for Solaris
#FC=f77
#FFLAGS=-fast -O5
all: lib$(LIB).a shared clean
...
# --------------------------------------------------------------------------------
catme:
	cat c*.f d*.f e*.f p*.f r*.f s*.f z*.f > All.f
lib$(LIB)All.a: All.o
	ar -rcs lib$(LIB)All.a All.o
testAll: test.o All.o lib$(LIB)All.a
	$(FC) test.o -Wl,-static -L./ -l$(LIB)All -Wl,-Bdynamic
	time ./a.out
...
test: test.o
	$(FC) test.o -Wl,-static -L./ -l$(LIB) -Wl,-Bdynamic
	time ./a.out
